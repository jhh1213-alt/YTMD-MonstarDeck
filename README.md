# YTMD Monstar Deck Plugin

Control [Pear Desktop](https://github.com/pear-devs/pear-desktop) from Monstar Deck (Stream Deck SDK compatible).
...
2. **Monstar Deck** (or another Stream Deck SDK 2 host, minimum software version 6.4).
3. Windows 10+ or macOS 10.11+.
4. Pear Desktop and Monstar Deck running on the **same computer**, unless you set Host to another machine that can reach the API Server.
...
Do not copy `src/`, `node_modules/`, or this Git repository into Monstar Deck. Only the `.sdPlugin` folder is the plugin.

### 3. Copy it into the Monstar Deck plugins folder

1. Quit Monstar Deck completely (not just the window — exit the app / tray icon).
2. Copy `fun.hlabs.ytmd.sdPlugin` into the plugins folder for your OS:

**Windows**

```text
%AppData%\Roaming\HotSpot\StreamDock\plugins
```

In File Explorer, paste that path into the address bar and press Enter. The result should look like:

```text
C:\Users\<your-user>\AppData\Roaming\HotSpot\StreamDock\plugins\fun.hlabs.ytmd.sdPlugin
```
...
In Finder, press **Shift + Command + G**, paste that path, and press Enter. The result should look like:

```text
/Users/<your-user>/Library/Application Support/HotSpot/StreamDock/plugins/fun.hlabs.ytmd.sdPlugin
```

The folder name must stay `fun.hlabs.ytmd.sdPlugin` (including the `.sdPlugin` suffix).

3. Start Monstar Deck again (or use **Reload plugins** if the app provides it).
4. In the action catalog, look for the **YTMD Connector** category.

If the category does not appear, confirm the folder is directly inside the plugins directory for your OS (not nested like `plugins\fun.hlabs.ytmd.sdPlugin.zip\fun.hlabs.ytmd.sdPlugin`) and that `manifest.json` is at the top of that folder.

- Windows: `%AppData%\Roaming\HotSpot\StreamDock\plugins`
...
5. Open the API Server settings:
   - **Hostname:** `127.0.0.1` if Monstar Deck is on the same PC (use `0.0.0.0` only if you know you need LAN access).
...
1. In Monstar Deck, drag **Play-Pause** (or any YTMD Connector action) onto a key.
...
| --- | --- |
| Category missing in Monstar Deck | Plugin is in `%AppData%\Roaming\HotSpot\StreamDock\plugins\fun.hlabs.ytmd.sdPlugin` (Windows) or `~/Library/Application Support/HotSpot/StreamDock/plugins/fun.hlabs.ytmd.sdPlugin` (macOS), with `manifest.json` at the top. Reload or restart Monstar Deck. |
